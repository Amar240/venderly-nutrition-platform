...
6. **Meal uniqueness:** one live normal meal event per student + service date + meal type (DB partial unique index). Cashier undo retains and marks the original reversed, allowing an ordinary re-entry; reversed events never count. Override requires a documented, audited reason.
